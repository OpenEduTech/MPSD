...
def get_hotel_list(resp):
    PWD = __file__[:__file__.rfind('/')]
    with open(f"{PWD}/resp.txt","w") as f:
        f.write(resp.text)
    hotel_list = []
    full_list = []
    # print(resp.text)
...
if __name__ == '__main__':
    ...
    print(start)
    # https://hotels.ctrip.com/hotels/list?countryId=1&city=1&checkin=2023/06/22&checkout=2023/06/25&optionId=1&optionType=City&directSearch=0&display=%E5%8C%97%E4%BA%AC&crn=1&adult=2&children=0&searchBoxArg=t&travelPurpose=0&ctm_ref=ix_sb_dl&domestic=1&
